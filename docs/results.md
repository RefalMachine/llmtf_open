...
Лениво обнаруживаемые API-наблюдения `logprobs` и `detokenize` сохраняются в
provenance, но не участвуют в semantic fingerprint. Их наблюдаемое значение
может зависеть от того, выполнилась предыдущая задача или была взята из кеша.
Существующие schema-v2 artifacts со старым order-dependent fingerprint
проверяются по вложенному `run_config` и остаются cache-compatible.
...
- для намеренного пересчёта — `--force_recalc`; старый task total удаляется
  перед запуском, поэтому неуспешный пересчёт не оставляет активного total.
...
`evaluation_results.txt` является сводкой всего output-каталога: при каждой
пересборке в него попадают все найденные завершённые `_total.jsonl`, включая
результаты других task groups и предыдущих вызовов в ту же директорию. Поэтому
не смешивайте в одном output-каталоге разные модели или несовместимые запуски.
Benchmark runner дополнительно пересобирает таблицу после завершения всех
worker-процессов.
