...
def load_llmtf_results(llmtf_results_path):
    ...
    if not isinstance(data, list):
        raise ValueError('LLMTF sample results must be a JSON array')
    return [sample['metric']['score'] for sample in data]
...
if __name__ == '__main__':
    ...
    from llmtf.tasks.llm_as_a_judge import (
        LLMAsJudgeGenStyleControl,
        LLMAsJudgeStyleControl,
        confident_score_mean_with_ties_and_ci,
    ...
    if args.judge_api_key is not None:
        ...
        parser.error(
            'set OPENAI_API_KEY or pass --judge_api_key for a local placeholder'
        )
    evaluator = Evaluator()
    
    model = LLM(backend=APIBackend(
        api_base=args.judge_base_url,
        model_context_len=args.max_len,
    ))
    ...
    if curr_battles_path in previous_battles_path and not args.force_recalc:
        ...
        model.generation_config.max_new_tokens = task.max_task_new_tokens
        output_dir = f'benchmark/llmaaj/{args.benchmark_name}/judges/{args.judge_model_name}/battles_raw/{args.model_name}'
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        evaluator = Evaluator()
        task_registry_name = f'llmaaj/{args.model_name}'
        evaluator.add_new_task(task_registry_name, TaskClass, task_params)
        summary = evaluator.evaluate(
            model=model,
            output_dir=output_dir,
            datasets_names=[task_registry_name],
            few_shot_count=0,
        ...
        if not summary.ok:
            raise RuntimeError(f'LLM-as-a-Judge evaluation failed: {summary.failed}')
