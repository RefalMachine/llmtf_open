...
def load_dataset(dataset_path, test=False):
    ...
    #val = [d for d in data if d['sent_id'] in val_sent_ids]
    print(Counter([d['type'] for d in val[:10]]))
    print(Counter([d['type'] for d in val[:5]]))
...
class RuOpinionNE(SimpleFewShotHFTask):
    def __init__(self, instruction, test=False, **kwargs):
    ...
    def dataset_args(self, test=False) -> Dict:
        if not test:
            return {'dataset_path': '../RuOpinionNE-2024/train.jsonl', 'test': test}
        else:
            return {'dataset_path': '../RuOpinionNE-2024/validation.jsonl', 'test': test}
    ...
    def _validate_dict(self, opinion, text):
        #Source, Target, Polarity, Expression.
    ...
    def create_messages(self, sample, with_answer=False, full_instruct=True) -> List[Dict]:
        full_instruction = '''Твоя задача состоит в том, чтобы проанализировать ***Текст*** и извлечь из него выражения мнений, представленные в виде кортежа мнений, состоящих из 4 основных составляющих:
...
1. Если в тексте нет выражения позитивных или негативных мнений, возвращай пустой список.
2. Если в тексте несколько разных мнений, то возвращай их все, каждый отдельным элементом списка.
3. При извлечении информации из текста, обязательно копируй ее полностью в том виде и форме, в котором она представлена в тексте.
4. Кортеж мнений обязан включать в себя Target, Polarity, Expression. В случае, если мнение выражает автор текста, то содержимое Source = AUTHOR, если источник мнения не автор и не содержится в тексте, то Source = NULL.
...
def evaluate_model(model_name_or_path, conv_path, output_path, few_shot_count, device_map='cuda:0', vllm=False):
    ...
    task = RuOpinionNE('', test=True)
    evaluator = Evaluator()
    evaluator.evaluate_dataset(task, model, output_path, 8000, few_shot_count, None, 32, 128000000000)
