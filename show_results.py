...
def read_jsonl(path):
    """Read current JSON-array samples and historical concatenated objects."""
    ...
    if isinstance(decoded, dict):
        ...
        raise ValueError(f"Expected a JSON array or object sequence in {path}")
    if not all(isinstance(record, dict) for record in records):
        raise ValueError(f"Every sample record must be an object in {path}")
    return records
...
def bootstrap(aggregation, metrics, n_bags, n_samples, leaderboard_aggregation, pool=None, task_name="unknown"):
    ...
    mean = statistics.fmean(aggrigations)
    std = statistics.pstdev(aggrigations)
    return (mean, std)
...
def save_table(
        ...
        for model in model_bench_to_score.keys():
            ...
            for cat in cat_headers:
                ...
                if scores:
                    ...
                    avg_std = math.sqrt(sum_var) / len(scores)
            ...
            if cat_vals:
                ...
                mean_std = math.sqrt(sum_var) / len(cat_vals)
    ...
    for model in model_bench_to_score.keys():
        ...
        if scores:
            ...
            sum_var = sum(s**2 for s in stds)
            mean_std = math.sqrt(sum_var) / len(scores)
            ...
            for total_path in model_dir.glob("*_total.jsonl"):
                ...
                if allow_bootstrap and do_bootstrap:
                    ...
                    if not log:
                        ...
                        continue
                    metrics = [sample["metric"] for sample in log]
                    try:
                ...
                if time_value is not None:
                    model_bench_to_score[model_name][task_name]["time"] = time_value
